...
async def save_analysis(user_id: str, filename: str, analysis_result: Dict) -> Optional[str]:
    ...
    if not headers:
        logger.warning("Supabase not configured (missing SUPABASE_URL/SUPABASE_KEY) — skipping save")
        return None
...
async def get_user_history(user_id: str) -> List[Dict]:
    headers = _get_headers()
    if not headers:
        logger.warning("Supabase not configured (missing SUPABASE_URL/SUPABASE_KEY) — returning empty history")
        return []
