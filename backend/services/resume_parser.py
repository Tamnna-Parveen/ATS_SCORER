...
def validate_file(file_data: bytes, filename: str) -> Tuple[bool, str, Optional[str]]:
    ...
    if file_size_bytes == 0:
        return False, 'Upload file is empty .... please check the file you have uploaded and try again', None
...
def extract_text_from_pdf(file_data:bytes) -> str:
    try:
        ...
        raise FileParsingError(
            ...
            'The PDF may corrupted, password-protected, or contain only scanned images. '
            'Please ensure it contains selected text.'
        ) from e
