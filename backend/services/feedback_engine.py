...
def analyze_issues(
    ...
    has_projects_signal = any(kw in resume_lower for kw in [
        ...
        'created a', 'implemented a', 'love demo', 'teck stack',
    ])

    if not proj_entries and len(projects) == 0 and not has_projects_signal:
        detected.append(IssueDetail(
            issue_title='Missing Project Section',
            severity_level="High",
            ats_impact="High",
            explanation=(
                "Your resume does not have a dedicated Projects section."
                "ATS system and recruiters look for concrete concrete projects to validate "
                "that your listed skills have been applied in practice."
            ),
            where_it_appears="Resume structure — no 'Projects' section found",
            how_to_fix=(
            ...
            action_items=[
                ...
                "Example: 'E-Commmerce Platform - React, Node.js, MongoDb. Handled 500+ transactions/month'",
            ...
            example_improvement=(
                ...
                "achieving 92% accuracy. Deployed as Rest API with FastAPI."
            ),
        ))
