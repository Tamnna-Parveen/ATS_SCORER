...
# Clean the URL defensively — strips stray whitespace or accidental quote characters
_CLEAN_SUPABASE_URL = (SUPABASE_URL or '').strip().strip('"').strip("'")
...
def _get_jwks_client() -> jwt.PyJWKClient | None:
    ...
    print(f"DEBUG: raw SUPABASE_URL = {repr(SUPABASE_URL)}")
    print(f"DEBUG: cleaned SUPABASE_URL = {repr(_CLEAN_SUPABASE_URL)}")

    if not _CLEAN_SUPABASE_URL:
        return None

    jwks_url = f"{_CLEAN_SUPABASE_URL.rstrip('/')}/auth/v1/.well-known/jwks.json"
...
def get_current_user(
    ...
    if not _CLEAN_SUPABASE_URL and not SUPABASE_JWT_SECRET:
        logger.error('Neither SUPABASE_URL (for JWKS) nor SUPABASE_JWT_SECRET configured — cannot verify tokens')
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail='Auth not configured on the server',
